...
export default async function SchedulePage({
  ...
  const { q, category, status, sort, page } = await searchParams;
  ...
  type Schedule = Awaited<ReturnType<typeof query.range>>["data"] extends (infer T)[] | null ? T : never;
  let schedules: Schedule[] | null;
  let totalPages = 1;

  if (status === "overdue") {
    // overdue is computed client-side, so fetch all matching rows and paginate in memory
    const { data: allSchedules } = await query;
    const filtered = (allSchedules ?? []).filter((s) => isOverdue(s));
    totalPages = Math.max(Math.ceil(filtered.length / PAGE_SIZE), 1);
    const start = (currentPage - 1) * PAGE_SIZE;
    schedules = filtered.slice(start, start + PAGE_SIZE);
  } else {
    const start = (currentPage - 1) * PAGE_SIZE;
    ...
    <div className="mx-auto max-w-6xl p-6 space-y-6">
      ...
      <div className="flex flex-col lg:flex-row gap-6">
        <div className="lg:w-80 shrink-0">
          <form action={addSchedule} className="space-y-3 rounded border p-4 lg:sticky lg:top-6">
            ...
            <button className="w-full rounded bg-black px-4 py-2 text-white dark:bg-white dark:text-black">
              추가
            </button>
          </form>
        </div>
        ...
        <div className="flex-1 space-y-4 min-w-0">
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-center gap-3 text-sm">
              {currentPage > 1 ? (
                <Link href={pageHref(currentPage - 1)} className="rounded border px-3 py-1">이전</Link>
              ) : (
                <span className="rounded border px-3 py-1 opacity-40">이전</span>
              )}
              <span>{currentPage} / {totalPages}</span>
              {currentPage < totalPages ? (
                <Link href={pageHref(currentPage + 1)} className="rounded border px-3 py-1">다음</Link>
              ) : (
                <span className="rounded border px-3 py-1 opacity-40">다음</span>
              )}
            </div>
          )}
        </div>
